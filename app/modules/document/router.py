...
@router.get("/", response_model=list[DocumentItem], status_code=status.HTTP_200_OK)
async def get_documents(user: str = Depends(get_current_user), session=Depends(get_db)):
    return await document_service.get_documents(user, session)
